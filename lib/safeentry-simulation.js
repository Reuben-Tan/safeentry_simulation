...
var currentTime = 0;
var statistics = [
  { name: "Time of day ", datetime: parseDate(datetime) },
  { name: "No. Customers in Store: ", count: 0,},
  { name: "No. Customers Infected: ", count: 0,},
  { name: "Avg % Customers Infected: ", count: 0,},
  { name: "R0: ", count: 0 },
  { name: "No of customers eating: ", count: 0 },
  { name: "No of customers shopping: ", count: 0 },
  { name: "Profits", count:0}
];
...
function updateStats() {
  ...
  document.querySelector("#statsprofits").innerHTML = statistics[7].count;
}
...
function initialiseData() {
  ...
  customers = [];
  statistics[1].count = 0; //No. of customers in store
  statistics[2].count = 0; //No. of customers infected
  statistics[3].count = 0; //Avg% of customers infected
  statistics[4].count = 0; // R0
  statistics[5].count = 0; //No. of customers eating
  statistics[6].count = 0 // No. of customers shopping
  statistics[7].count = 0 //Profits
  cumratioinfected = 0;
  totinfected = 0;
  totnewinfected = 0;
  queues = [[], [], []];
...
function updateCustomer(customerIndex) {
    ...
    while (customer.type == "N" && i < infectedcitizens.length) {
      ...
      if (distance < parameters.DistTransmission) {
        if (Math.random() < parameters.InfectionRate) {
          (customer.type = "NI"), statistics[2].count++, totnewinfected++;
        }
      }
      i = i + 1;
    }
  ...
  switch (state) {
    ...
    case "LEFT":
      if (hasArrived) {
        if (customer.type == "I" || customer.type == "NI") {statistics[2].count--}
        statistics[1].count--
        customer.state = "GONE";
      }
    default:
