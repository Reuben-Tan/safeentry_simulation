...
function updateCustomer(customerIndex) {
  ...
  if (customer.state == "N") {
    var infectedcitizens = customers.filter(function (d) {
      return d.type == "I" & d.place != 'outside mall' & d.place == customer.place;
    });
...
function updateDynamicAgents(redraw = true) {
  ...
  for (var customerIndex in customers) {
    if (customers[customerIndex].state != 'GONE') {updateCustomer(customerIndex);}
  }
  if (redraw) {
    updateSurface();
  }
...
function simStep(fastfwd = false) {
  if (isRunning) {
    ...
    // update stats
    currentTime++;
    statistics.totnewinfected = customers.filter(function (d) {return d.type == "NI"}).length;
    statistics.totinfected = customers.filter(function (d) {return (d.type == "NI" || d.type == "I")}).length;
    statistics.timeofday = parseDate(datetime);
