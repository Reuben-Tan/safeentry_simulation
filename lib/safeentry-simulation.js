...
function simStep(fastfwd = false) {
  if (isRunning) {
    ...
    console.log(statistics.timeofday);
...
function generateData() {
  const simulationResults = [Object.keys(statistics).concat(Object.keys(statistics.infectionLocation))];
  var simCount = 2
  var i = 0
  while (i < simCount) {
    initialiseData();
    ...
    // simulationResults.push(statistics)
    simulationResults.push(Object.values(statistics).concat(Object.values(statistics.infectionLocation)))
    i++
  }
  let csvContent = ''
