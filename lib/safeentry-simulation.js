...
function updateStats() {
  ...
  document.querySelector('#rest-count').innerHTML = customers.filter(function (d) {return ['food0','food1'].includes(d.place)}).length
  document.querySelector('#retail-count').innerHTML = customers.filter(function (d) {return ['shop0','shop1','shop2','shop3'].includes(d.place)}).length
  document.querySelector('#mall-count').innerHTML = customers.filter(function (d) {return ['mall'].includes(d.place)}).length
  document.querySelector('#outside-count').innerHTML = customers.filter(function (d) {return ['Outside mall'].includes(d.place)}).length
...
function initialiseData() {
  ...
  customers = [];
  statistics.customers = 0; //No. of customers in store
  statistics.infected = 0; //No. of customers infected
  statistics.avgInfected = 0; //Avg% of customers infected
  statistics.r0 = 0; // R0
  statistics.profits = 0; //Profits
  cumratioinfected = 0;
  totinfected = 0;
  totnewinfected = 0;
  queues = [[], [], []];
...
function addDynamicAgents() {
  if (Math.random() < probArrival[datetime.getHours()]) {
    ...
    if (Math.random() < parameters.probInfected) {
      (newcustomer.type = "I"), statistics.infected++, totinfected++;
    } else {
      newcustomer.type = "N";
    }
...
function updateCustomer(customerIndex) {
    ...
    while (customer.type == "N" && i < infectedcitizens.length) {
      ...
      if (distance < parameters.DistTransmission) {
        if (Math.random() < parameters.InfectionRate) {
          (customer.type = "NI"), statistics.infected++, totnewinfected++;
        }
      }
      i = i + 1;
    }
  ...
  switch (state) {
    ...
    case "LEFT":
      if (hasArrived) {
        if (customer.type == "I" || customer.type == "NI") {
          statistics.infected--;
        }
        statistics.customers--;
        customer.state = "GONE";
...
function simStep(fastfwd = false) {
  if (isRunning) {
    ...
    statistics.r0 = totnewinfected / totinfected;
    statistics.ratio = statistics.profits / statistics.infected;
    if (!fastfwd) {
      updateChart();
      updateStats();
    }
