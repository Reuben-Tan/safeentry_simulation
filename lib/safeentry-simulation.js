...
function toggleSimStep() {
  isRunning = !isRunning;
  if (isRunning){
    document.querySelector('#startsim').value = "Pause"
  } else {
    document.querySelector('#startsim').value = "Start"
  }
}
...
function redrawWindow() {
  ...
  statistics[0].count = parseDate(datetime);
  customers = [];
  ...
  var surfaceHeight = h - 50 - 3 * WINDOWBORDERSIZE;

  drawsurface.style.width = "-webkit-fill-available";
  drawsurface.style.height = "-webkit-fill-available";
...
function updateSurface() {
  ...
  images
    ...
    .attr("xlink:href", function (d) {
      if (d.type == "I") {return urlInfected;}
      else {console.log(d.id); return urlcustomers[d.id % 4];}
    })
  ...
  var newkeyarea = allkeyareas.enter().append("g").attr("class", "keyarea");
  newkeyarea
    .append("svg:image")
    .attr("x", function (d) {
      var cell = getLocationCell(d.location);
