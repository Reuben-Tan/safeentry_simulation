...
function updateSurface() {
  ...
  newlayout
    .append("rect")
    .attr("x", cellWidth * nodes.queueStartingPoint[0][0])
...
function updateCustomer(customerIndex) {
  ...
  var hasArrived =
    Math.abs(customer.target.x - x) + Math.abs(customer.target.y - y) ==
    0;
  ...
  if (infectedcitizens.length > 0 && customer.type == "N") {
    while (customer.type == "N" && i < infectedcitizens.length) {
      ...
      var distance = Math.sqrt(
        (infectedx - x) * (infectedx - x) +
          (infectedy - y) * (infectedy - y)
      );
  ...
  var newx =
    x + Math.min(Math.abs(deltax), cellsPerStep) * Math.sign(deltax);
  var newy =
    y + Math.min(Math.abs(deltay), cellsPerStep) * Math.sign(deltay);
