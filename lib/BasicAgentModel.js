...
var caregivers = [
	...
	{"type":RECEPTIONIST,"label":"Exit","location":{"row":exitRow,"col":exitCol},"state":IDLE},
	{"type":ENTRANCE,"label":"Entrance","location":{"row":entranceRow,"col":entranceCol},"state":IDLE}
];
...
function addDynamicAgents(){
	...
	if (Math.random()< probArrival){
		...
		"target":{"row":entranceRow,"col":entranceCol},"state":QUEUEING,"timeAdmitted":0,"shoppingTime":20,"queuingtime":5};
	...
	switch(state){
		...
		case SHOPPING:
			if (shoptime > 0) {
				...
				patient.target.row = Math.floor(Math.random() * 9) + 3  ;
				patient.target.col = Math.floor(Math.random() * 16) + 12;
			}
			else{
...
function removeDynamicAgents(){
	...
	var treatedpatients = allpatients.filter(function(d,i){return d.state==LEFT;});
	...
	patients = patients.filter(function(d){return d.state!=LEFT;});
